...
import { WEEKLY_PLAN_LIMIT, PLAN_LABEL, weeklyRemainingLabel } from "@/lib/plan-limits";
...
export function UsageBadge({ compact = false }: { compact?: boolean }) {
  const { data } = useUsage();
  if (!data) return null;
  const label = weeklyRemainingLabel(data.tier, data.monthlyPlansUsed);
  const unlimited = WEEKLY_PLAN_LIMIT[data.tier] === null;
  return (
    <Link
      to="/pricing"
      className={
    ...
    >
      ...
      {compact ? (unlimited ? "Unlimited" : label.replace("Weekly Plan Generations: ", "")) : label}
